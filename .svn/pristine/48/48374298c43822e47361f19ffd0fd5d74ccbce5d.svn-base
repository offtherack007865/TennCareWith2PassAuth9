    ...
    public class CallWebApiLandClass
    {
            ...
            qy_GetDunningToLookUp()
        {
            qy_GetDunningToLookUpOutput
                returnOutput =
                    qy_GetDunningToLookUpAsync()
                    .Result;

            return returnOutput;
        }

        public async Task<qy_GetDunningToLookUpOutput>
                        qy_GetDunningToLookUpAsync()
        {
            log.Info($"In qy_GetDunningToLookUpAsync");
            qy_GetDunningToLookUpOutput
                returnOutput =
                    new qy_GetDunningToLookUpOutput();

            string myCompleteUrl = $"{MyBaseWebApiUrl}/api/Ops/qy_GetDunningToLookUp";
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var result = await client.GetAsync(myCompleteUrl);
                    var response = await result.Content.ReadAsStringAsync();
                    returnOutput = JsonConvert.DeserializeObject<qy_GetDunningToLookUpOutput>(response);
                }
            }
            catch (Exception ex)
            {
            ...
            qy_GetDunningEligibility()
        {
            qy_GetDunningEligibilityOutput
                returnOutput =
                    qy_GetDunningEligibilityAsync()
                    .Result;

            return returnOutput;
        }

        public async Task<qy_GetDunningEligibilityOutput>
                        qy_GetDunningEligibilityAsync()
        {
            log.Info($"In qy_GetDunningEligibilityAsync");
            qy_GetDunningEligibilityOutput
                returnOutput =
                    new qy_GetDunningEligibilityOutput();

            string myCompleteUrl = $"{MyBaseWebApiUrl}/api/Ops/qy_GetDunningEligibility";
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var result = await client.GetAsync(myCompleteUrl);
                    var response = await result.Content.ReadAsStringAsync();
                    returnOutput = JsonConvert.DeserializeObject<qy_GetDunningEligibilityOutput>(response);
                }
            }
            catch (Exception ex)
            {
            ...
            dd_CollectionsEligibility()
        {
            dd_CollectionsEligibilityOutput
                returnOutput =
                    dd_CollectionsEligibilityAsync()
                    .Result;

            return returnOutput;
        }

        public async Task<dd_CollectionsEligibilityOutput>
                        dd_CollectionsEligibilityAsync()
        {
            log.Info($"In dd_CollectionsEligibilityAsync");
            dd_CollectionsEligibilityOutput
                returnOutput =
                    new dd_CollectionsEligibilityOutput();

            string myCompleteUrl = $"{MyBaseWebApiUrl}/api/Ops/dd_CollectionsEligibility";
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var result = await client.GetAsync(myCompleteUrl);
                    var response = await result.Content.ReadAsStringAsync();
                    returnOutput = JsonConvert.DeserializeObject<dd_CollectionsEligibilityOutput>(response);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (inputImportFileName.Trim().Length == 0)
            {
                inputImportFileName = "BLANK";
            }
            if (inputDOBInyyyyMMdd.Trim().Length == 0)
            {
                inputDOBInyyyyMMdd = "19000101";
            }
            if (inputSSN.Trim().Length == 0)
            {
                inputSSN = "BLANK";
            }
            if (inputDOSInyyyyMMdd.Trim().Length == 0)
            {
                inputDOSInyyyyMMdd = "19000101";
            }
            if (inputDateImportedInyyyyMMdd.Trim().Length == 0)
            {
                inputDateImportedInyyyyMMdd = "19000101";
            }
            if (inputMCO.Trim().Length == 0)
            {
                inputMCO = "BLANK";
            }
            if (inputMedicare.Trim().Length == 0)
            {
                inputMedicare = "BLANK";
            }
            if (inputPCP.Trim().Length == 0)
            {
                inputPCP = "BLANK";
            }
            ...
            if (inputTennCareEligibility.Trim().Length == 0)
            {
                inputTennCareEligibility = "BLANK";
            }
            if (inputOutputFileDateInyyyyMMdd.Trim().Length == 0)
            {
                inputOutputFileDateInyyyyMMdd = "19000101";
            }
            if (inputOutputFileName.Trim().Length == 0)
            {
                inputOutputFileName = "BLANK";
            }
                        ...
                        )
        {
            ...
            if (inputPatientIDString.Trim().Length == 0)
            {
                inputPatientIDString = "BLANK";
            }
            if (inputDOBInyyyyMMdd.Trim().Length == 0)
            {
                inputDOBInyyyyMMdd = "19000101";
                        ...
                        )
        {
            ...
            if (inputPatientName.Trim().Length == 0)
            {
                inputPatientName = "BLANK";
            }
            if (inputDOBInyyyyMMdd.Trim().Length == 0)
            {
                inputDOBInyyyyMMdd = "19000101";
            ...
            if (inputImportFullFileName.Trim().Length == 0)
            {
                inputImportFullFileName = "BLANK";
            }
            if (inputDateImportedInyyyyMMdd.Trim().Length == 0)
            {
                inputDateImportedInyyyyMMdd = "19000101";
